...
# Under work
        ...
        if subb == '':
            ...
            def create_csv():
                ...
                csv_name = 'E:\\Face-Recognition-Attendance-System-main\\Attendance\\Manually Attendance\\'+DB_table_name+'.csv'
            ...
            def attf():
                import subprocess
                subprocess.Popen(
                    r'explorer /select,"E:\\Face-Recognition-Attendance-System-main\\Attendance\\Manually Attendance\\-------Check atttendance-------"')
...
def take_img():
    l1 = txt.get()#store the enrollment number entered by the user
    l2 = txt2.get()#store the name entered by the user
    if l1 == '':#check if any label is empty or not, if found empty then call the function 'err_screen()' which will print name and enrollment number required
        err_screen()
    elif l2 == '':
        err_screen()
    else:
        try:
            cam = cv2.VideoCapture(0)#Use cv2.VideoCapture() to get a video capture object for the camera.
#Set up an infinite while loop and use the read() method to read the frames using the above created object.

# for face detection we use "HAARCASCADE" algorithm as it is fast, computationally less expensive and gives high accuracy.
            ...
            while (True):
                ret, img = cam.read()
                gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)#Converting the image to grayscale(the image is a three-layer image (i.e., RGB), So It is converted to a one-layer image (i.e., grayscale). )
                faces = detector.detectMultiScale(gray, 1.3, 5)# Applying the face detection method on the grayscale image(returns boundary rectangles for the detected faces), {scaleFactor and minNeighbors}
                for (x, y, w, h) in faces:#Iterating through rectangles of detected faces(Rectangles are drawn around the detected faces by the rectangle method of the cv2 module)
                    ...
                    cv2.imshow('Frame', img)#cv2.imshow() method is used to display an image in a window. The window automatically fits the image size.
                # wait for 100 miliseconds,  allows users to display a window for given milliseconds or until any key is pressed.
                
                #  0xFF in this scenario is representing binary 11111111 a 8 bit binary, since we only require 8 bits to represent a character we AND waitKey(0) to 0xFF. As a result, an integer is obtained below 255.
            ...
            with open('StudentDetails\StudentDetails.csv', 'a+') as csvFile:#open csv file in append mode
                writer = csv.writer(csvFile, delimiter=',')
                writer.writerow(row)#This function writes items in a sequence (list, tuple or string) separating them by comma character.
                csvFile.close()
        ...
        if time.time() < future:
            if sub == '':
                ...
                while True:
                    ...
                    # 1. Capturing frames from the video capture object.
                    # 2. Convert the images to gray scale.
                    # 3. Detect face in the object.
                    # 4. Recognize the Id from the available data in trainer.yml file.
                    # 5. Create a rectangle over the face and print the ID of the person detected over the rectangle.
                    ret, im = cam.read()
...
# After face is recognized and attendance is being updated then a new window will open and display that subject attendance with the student details.
                ...
                root.configure(background='grey80')
                cs = "E:\\Face-Recognition-Attendance-System-main\\" + fileName
    ...
    def Attf():
        ...
        # for checking sheets a subprocess is created
        
        subprocess.Popen(
            r'explorer /select,"E:\\Face-Recognition-Attendance-System-main\\Attendance\\-------Check atttendance-------"')
    ...
    def log_in():
        ...
        if username == 'admin':
            if password == 'yajnira':
                ...
                cs = 'E:\\Face-Recognition-Attendance-System-main\\StudentDetails\\StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
...
# For train the model
# here in this module we are generating a trainer (named TrainingImageLabel) which will further generate a file Trainner.yml for our face recognizer
def trainimg():
    # In LBP, a binary code is generated at each pixel by thresholding it’s neighbourhood pixels to either 0 or 1 based on the value of the centre pixel.
    # LBP value of image which is to be recognized is to be compared with all the LBP values of the stored images and Euclidean distance is calculated where it is 0 match found.
    recognizer = cv2.face.LBPHFaceRecognizer_create()
    # OpenCV transforms LBP images to histograms to store spatial information of faces
    # During the inference, it calculates the test face's LBP, divides the regions and creates a histogram. Then compares its euclidian distance to the trained faces' histograms.
    global detector
    detector = cv2.CascadeClassifier("haarcascade_frontalface_default.xml")
    try:
        global faces, Id
        
        # will load the training images from dataSet 
        # a. Load the training images from dataSet folder
        # b. capture the faces and Id from the training images
        # c. Put them In a List of Ids and faceSamples and return it.
...
# we have the image we will train over the images 
    recognizer.train(faces, np.array(Id))
    try:
        recognizer.save("TrainingImageLabel\Trainner.yml")#we will also give the path to save our file that will be generated after training.
    except Exception as e:
...
def getImagesAndLabels(path):
    # image path is created and this will create the path for image in the folder
...
# For creating a label asking for enrollment in the home page
lbl = tk.Label(window, text="Enter Enrollment : ", width=20, height=2,
               fg="black", bg="grey", font=('times', 15, 'bold'))
lbl.place(x=200, y=200)

# for keeping a check that only numeric digits can be entered in the enrollment field
def testVal(inStr, acttyp):
    if acttyp == '1':  # insert
        if not inStr.isdigit():
            return False
...
# For creating a label asking for name in the home page
...
# On hitting 'Take Images' button it will do the command as specified in the 'take_img'
